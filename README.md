...
- Color parsing (named, hex, `rgb()`/`rgba()`, `hsl()`/`hsla()`, `hwb()`, `lab()`, `lch()`, `oklab()`, `oklch()`, `color()` for the sRGB/P3/XYZ spaces, and `color-mix()` in any of those spaces with hue methods), including `none` components and modern space/slash syntax; results are clipped to sRGB
...
- **Color**: colours resolve to 8-bit sRGB with per-channel clipping (no CSS gamut mapping). `calc()` inside colour components, the relative colour syntax (`from`), `light-dark()`, system colours, and the `a98-rgb` / `prophoto-rgb` / `rec2020` spaces are not parsed.
