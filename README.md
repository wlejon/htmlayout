...
- Color parsing (named, hex, `rgb()`/`rgba()`, `hsl()`/`hsla()`, `hwb()`, `lab()`, `lch()`, `oklab()`, `oklch()`, `color()` for the sRGB/P3/A98/ProPhoto/Rec.2020/XYZ spaces, and `color-mix()` in any of those spaces with hue methods), including `none` components, modern space/slash syntax, `calc()`/`min()`/`max()`/`clamp()` in components, the relative colour syntax (`rgb(from <color> r g calc(b / 2))` for every function), and `light-dark()` resolved against `ColorContext::scheme` (`usedColorScheme()` derives it from `color-scheme` and the preferred scheme); out-of-gamut results are gamut-mapped into sRGB with the CSS Color 4 OKLCH chroma-reduction algorithm
...
- CSS nesting: nested style rules with `&` (anywhere, including `&.x` and `.a &`), relative nested selectors (`> .c`, `.c`), selector lists at every level, nested `@media` / `@supports` / `@container` / `@layer` (block and statement forms; nested layer names qualify as `outer.inner`), `@media` inside `@container` and `@container` inside `@media`, a top-level `&` matching as `:scope` (the root element), and declarations interleaved with nested rules — desugared into flat rules at parse time (see `src/css/nesting.cpp` for the specificity caveat). The flat `Stylesheet` cannot express an `@container` inside an `@layer` (or vice versa) or two nested container queries; those inner rules are dropped
...
- `line-clamp` (`none | auto | [<integer> || <block-ellipsis>] -webkit-legacy?`, a shorthand for the `max-lines` / `block-ellipsis` (`auto` / `no-ellipsis` / a `<string>`, inherited) / `continue` longhands, which is what layout reads: a box clamps when `continue` is `collapse` (or `discard` / `-webkit-legacy`) and `max-lines` is an integer) and the legacy `-webkit-line-clamp` on `display: -webkit-box; -webkit-box-orient: vertical` (or its inline-level counterpart `-webkit-inline-box`, laid out as an inline-block); both apply to inline-blocks as well as blocks: lines are counted through in-flow block descendants in the same formatting context, the box's auto height ends after the Nth line, text past it is removed from the placed runs and element boxes past it are flagged `clampHidden`, and the ellipsis goes at the last kept line's inline end (right in `ltr`, left in `rtl`), the line truncated from that edge in visual order as Chromium does — text runs cut from the side facing the edge, atomic inlines kept whole when the ellipsis fits after them and flagged `clampHidden` when it does not. The ellipsis is drawn through `textRuns`: appended to the cut run, or (after an atomic inline, or beside a run against the line's direction) as a run of its own in a text node
...
- **Color**: colours resolve to 8-bit sRGB (wide-gamut colours are gamut-mapped, not preserved). System colours (`Canvas`, `ButtonText`, …) are not parsed. `light-dark()` is resolved by the consumer's `tryParseColor()` call, so the consumer passes each element's used colour scheme in the `ColorContext`; the cascade leaves the function in the computed value.
